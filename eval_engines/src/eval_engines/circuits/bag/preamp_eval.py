...
class PreAmpEvaluationEngine(BagEvalEngine):
    ...
    def impose_constraints(self, design_dict):
        ...
        for kwrd in design_dict.keys():
            ...
            if step_2_indicator.startswith('seg_dict') or step_2_indicator.startswith('w_dict') or step_2_indicator.startswith('n'):
...
if __name__ == '__main__':
    ...
    fname = 'specs_design/PreAmp.yaml'
    evalEngine = PreAmpEvaluationEngine(design_specs_fname=fname)
    content = read_yaml(fname)
    dir = content['database_dir']

    start = time.time()
    sample_designs = evalEngine.generate_data_set(n=5, evaluate=True)
