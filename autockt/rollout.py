...
from envs.spectre_fc import FoldedCascode 
...
register_env("fc-v0", lambda config:FoldedCascode(config))
...
def rollout(agent, env_name, num_steps, out="assdf", no_render=True):
    ...
    while rollout_steps < args.num_val_specs:
        ...
        reached_spec = unlookup(state[0:spec_num], norm_spec_ref)
        if done == True:
            reached_specs += 1
            agent_conv.append(reached_spec)
        ...
        pickle.dump(obs_reached, open("obs_reached_test","wb"))
        pickle.dump(agent_conv, open("agent_conv_act","wb"))
        #pickle.dump(agent_nc, open("agent_nc_act", "wb"))
        pickle.dump(obs_nreached, open("obs_nreached_test","wb"))
        print("Specs reached: " + str(reached_specs) + "/" + str(len(obs_nreached))) 
